        ...
        <div className="max-w-md w-full text-center">
          ...
          <div className="space-y-3">
            ...
            <div id="google-signin-button" className="w-full min-h-[48px] flex items-center justify-center">
              {/* Fallback button - Always show until GIS renders its button */}
              <button
                onClick={async () => {
                  setLoginLoading(true);
                  setLoginError('');
                  try {
                    // Check if GIS is loaded and initialized
                    if ((window as any).google && (window as any).google.accounts && (window as any).google.accounts.id) {
                      // Try to trigger one-tap sign-in
                      (window as any).google.accounts.id.prompt((notification: any) => {
                        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
                          ...
                          (window as any).google.accounts.oauth2.initTokenClient({
                            ...
                            scope: 'email profile',
                            callback: async (response: any) => {
                              // This won't work directly, we need idToken
                      ...
                      setLoginError('Google Sign-In is loading. Please wait...');
                      // Retry after delay
                      setTimeout(() => {
                        if ((window as any).google && (window as any).google.accounts) {
                ...
                style={{ display: 'none' }} // Hide by default, GIS will replace it
              >
