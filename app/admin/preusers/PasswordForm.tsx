 ...
 * Handles form submission for admin preusers panel access
 */
export default function PasswordForm() {
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/admin/preusers/auth', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        window.location.href = '/admin/preusers';
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center p-4">
      <div className="bg-[#1a1a1a] p-8 rounded-lg border border-gray-800 max-w-md w-full">
        <h1 className="text-2xl font-bold mb-2 text-center">👑 Admin Premium Users Panel</h1>
        <p className="text-center text-gray-400 mb-6">Premium üyeler paneline erişmek için şifre girin</p>
